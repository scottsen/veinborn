...
This is a template for creating custom actions in Veinborn.
...
-- Use veinborn.add_message() to inform the player why validation failed.
...
function validate(actor_id, params)
    ...
    if not params.target_x or not params.target_y then
        veinborn.add_message("Target coordinates required!")
        return false
    end
    ...
    if dist > PARAMETER_2 then
        veinborn.add_message("Target out of range!")
        return false
    end
...
function execute(actor_id, params)
    ...
    veinborn.modify_stat(actor_id, "mana", -PARAMETER_1)

    -- Example: Add messages
    veinborn.add_message("Action executed successfully!")

    -- Example: Affect entities
    local target_x = params.target_x
    local target_y = params.target_y
    local targets = veinborn.get_entities_in_range(target_x, target_y, 2)

    local events = {}
    for i = 1, #targets do
        local target = targets[i]
        if target.entity_type == "MONSTER" then
            veinborn.modify_stat(target.id, "hp", -10)
            table.insert(events, {
                type = "damage",
                target_id = target.id,
                amount = 10
    ...
    return {
        ...
        messages = {},         -- Additional messages (or use veinborn.add_message)
